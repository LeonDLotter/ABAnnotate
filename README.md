...
2. For each null phenotype and each gene category a correlation score is obtained by correlating the null phenotype with the spatial mRNA expression pattern of each gene and averaging the z-transformed correlation coefficient within each category (= null categories).  
3. The obtained correlation coefficients are then compared to the "real" category-wise correlation coefficients obtained by correlating the "real" phenotype with all genes in each category and averaging the correlation coefficients per category.  
4. One-sided p values for each category are obtained and the resulting p-values are FDR-corrected.  
...
opt.atlas = 'SchaeferTian'; % one of {'SchaeferTian', 'Neuromorphometrics'} 
opt.dir_result = '/path/to/save/output'; % output directory
opt.GCEA.dataset = 'GO-biologicalProcessProp-discrete'; % selected GCEA dataset

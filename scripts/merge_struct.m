function into = merge_struct(into, from)
  %MERGE_STRUCT merge all the fields of scalar structure from into scalar structure into
  ...
  for fn = fns.'
     if isstruct(from.(fn{1})) && isfield(into, fn{1})
          ...
          into.(fn{1}) = merge_struct(into.(fn{1}), from.(fn{1}));
     else
